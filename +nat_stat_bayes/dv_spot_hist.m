...
%   spot_llr = nat_stat_bayes.dv_spot_hist(abr_a, abr_b, patch_size, ...
...
%   nat_stat_bayes.multinomial_llr, and the stray email note is removed.)
%
%   Feature indices
...
%
%   See also NAT_STAT_BAYES.MULTINOMIAL_LLR, NAT_STAT_BAYES.DV_EDGE_HIST.
    ...
    for k = 1:3
        if feature_list(k) == 1
            ...
            spot_llr(k) = nat_stat_bayes.multinomial_llr(N1, N2);
        end
    end
    ...
    if any(feature_list(12:14) == 1)
        ...
        ch_a = vislib.cntrst_norm(abr_a(:, :, 1), target_contrast, patch_size);
        ch_b = vislib.cntrst_norm(abr_b(:, :, 1), target_contrast, patch_size);
        for s = 1:size(cs_spec, 1)
            k = cs_spec(s, 1);
            if feature_list(k) == 1
                cs_a = vislib.center_surround(ch_a, cs_spec(s, 2), cs_spec(s, 3));
                cs_b = vislib.center_surround(ch_b, cs_spec(s, 2), cs_spec(s, 3));
